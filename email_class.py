...
def print_multiple(attribute, input_list):
    """ Prints strings to stdout based on the content of the variable """
    if not input_list:
...
class Email:
    def __init__(self, file_path):
        ...
        self.output_path = os.path.splitext(self.filename)[0]
        # Headers
        self.from_h = None
        self.sender_h = None
    ...
    def print_to_file(self):
        ...
        with open(os.path.join(os.path.splitext(self.filename)[0], "analysis.txt"), "w") as of:
            try:
    ...
    def print_cli(self):
        """ Prints the list of most important analized email attributes to the stdout """
        print("File name:", self.filename)
        print("From:", self.from_h)
        print("Sender:", self.sender_h)
        print("Received-SPF:", self.received_spf)
        print(print_multiple("To:", self.to_h))
        print("CC:", self.cc_h)
        print("Return-Path:", self.return_path_h)
        print("Reply-To:", self.reply_to_h)
        print("Subject:", self.subject_h)
        print(print_multiple("URLs:", self.urls))
        print(print_multiple("IPs:", self.ips))
        print(print_multiple("Domains:", self.domains))
        if self.attachments:
            print("Attachments:")
            for value in self.attachments:
